...
PolygonGeometryDrawer::PolygonGeometryDrawer(PolygonGeometry* polygon, MatVec::Vector3 color, Game& game)
	:polygon_(polygon),color_(color),vertex_map_(nullptr),abs_v_(polygon->points_.size()),frame_(game)
{
...
void PolygonGeometryDrawer::Output(Game& game)
{
	...
	game.mdx12.SetVertexBuffers(vertex_buffer_,0,sizeof(XMFLOAT3)*abs_v_,sizeof(XMFLOAT3));
	game.mdx12.SetIndexBuffers(index_buffer_,3*(abs_v_-2));
	game.mdx12.SetViewports(900, 900, 0, 0, 0.0f, 1.0f);
	game.mdx12.SetScissorrect(0.0f, 900.0f, 0.0f, 900.0f);
	game.mdx12.DrawIndexedInstanced(3 * (abs_v_ - 2), 1, 0, 0, 0);
...
void PolygonGeometryDrawer::NonstaticGraphicInit(Game& game)
{
	...
	*color_map_ = MatVec::ConvertToXMFLOAT3(color_);
	...
	vertex_buffer_ = game.mdx12.CreateVertexBuffer(sizeof(XMFLOAT3) * abs_v_, L"PolygonGeometryDrawer::vertex_buffer_");
	vertex_map_ = static_cast<XMFLOAT3*>(game.mdx12.Map(vertex_buffer_));
	
	index_buffer_ = game.mdx12.CreateIndexBuffer(sizeof(unsigned int) * 3 * (abs_v_ - 2), L"PolygonGeometryDrawer::index_buffer_");
	unsigned int* index_map = static_cast<unsigned int*>(game.mdx12.Map(index_buffer_));
	for (unsigned int i = 2; i < abs_v_; i++)
	{
		index_map[3 * (i - 2) + 0] = 0;
